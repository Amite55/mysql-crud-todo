...
const Details = () => {
    ...
    const detailsData = async () => {
        try {
            const { data } = await axios.get(`http://localhost:8080/read/${id}`);
            setDetails(data);
        } catch (err) {
            console.log(err);
        }
        ...
        <>
            <div
                ...
                <div className="text-center mt-2">
                    <h2 className="font-semibold">{details[0]?.name}</h2>
                    <p className="text-gray-500">Email: {details[0]?.email}</p>
                    <p className="text-gray-500">Roll: {details[0]?.roll}</p>
                </div>
                ...
                <div className="p-4 border-t mx-8 mt-2">
                    <Link to={"/"}>
                        <button className="w-1/2 block mx-auto rounded-full bg-gray-900 hover:shadow-lg font-semibold text-white px-6 py-2">Back</button>
                    </Link>
                </div>
            </div>
        </>
